...
class ConnectionDataSpec: BaseSpec {
    override func spec() {
        describe("init?(code:,tag:)") {
            context("when key pair successfully created and public key data obtained") {
                it("should return ConnectionData") {
                    ...
                    expect(connectionData?.providerCode).to(equal(providerCode))
                    expect(connectionData?.publicKey).to(equal(expectedPublicKeyData))
                }
            }
        }
    }
